...
const handleEditNote = async(req,res) => {
    ...
    try {
        ...
        const noteIndex = userNotes.subjects.filter(sub => {
            return sub.name === subject }).pop().notes.findIndex(item => item.slug === req.params.slug)
        ...
        console.log(result) 
        if(result.modifiedCount === 0){ return res.status(400).send(`BAD_REQUEST: No Update.`) }   
